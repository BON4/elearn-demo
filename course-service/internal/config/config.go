...
type Config struct {
	...
	OutboxWorkerInterval time.Duration `env:"OUTBOX_WORKER_INTERVAL" default:"500ms"`
}
...
func LoadFromINI(path string) (*Config, error) {
	...
	intervalStr := file.Section("").Key("OUTBOX_WORKER_INTERVAL").MustString("500ms")
