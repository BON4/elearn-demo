...
type Config struct {
	...
	ConsumerInterval time.Duration `env:"CONSUMER_INTERVAL" default:"500ms"`
}
...
func (c *Config) Validate() error {
	...
	if c.ConsumerInterval == 0 {
		return errors.New("CONSUMER_INTERVAL is required")
	}
...
func LoadFromINI(path string) (*Config, error) {
	...
	intervalStr := file.Section("").Key("CONSUMER_INTERVAL").MustString("500ms")
