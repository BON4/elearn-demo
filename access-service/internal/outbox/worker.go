...
func (w *Worker) setupQueue() error {
...
func (w *Worker) publishEvent(ctx context.Context, evt *domain.OutboxEvent) error {
	...
	err = w.publishCh.PublishWithContext(ctx, CoursesExchange, routingKey, false, false, amqp.Publishing{
	...
	log.
		WithField("outbox_event_uuid", evt.ID).
		WithField("outbox_event_exchange", CoursesExchange).
